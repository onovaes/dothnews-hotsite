...
- Os icones sao a Material Symbols auto-hospedada em `src/fonts/material-symbols-outlined.woff2`, **subsetada por codepoint** e renderizada pelo **caractere** (mapa `src/lib/icons.js` via `iconChar(name)`), nao pela ligadura — isso mantem a fonte ~2KB (o subset por ligadura inchava p/ ~230KB). As fontes ficam em `src/fonts/` (processadas pelo Vite: a de icones vira data-URI no CSS inlinado, as IBM Plex ganham hash) — nao usar `public/` p/ fontes (perde cache-busting). Ao add/remover icone: atualize a lista `ICONS` em `scripts/build-icon-font.sh` e rode-o (regenera a fonte E o `src/lib/icons.js`). `font-display: swap` + guard `.js`/`fonts-ready` (`visibility:hidden`) evitam flash. Ao adicionar/remover um icone, atualize a lista e rode `scripts/build-icon-font.sh` para regenerar o subset. Nao reintroduzir Google Fonts com `display=optional` (causava o texto cru tipo `arrow_outward` no primeiro acesso). Ver `DESIGN.md`.
